...
export function Header(){
    ...
    return(
        <nav className="px-2 py-4 port-header sticky top-0 z-50 shadow-lg/10 bg-gray-600 light:bg-white shadow-black light:shadow-black text-white light:text-black">
                    ...
                    `}>
                        {
                            navbarLinks.map((item) => (
                                <li key={item.id}>
                                    <a onClick={() => setOpen(false)} className={linkStyles} href={item.url} {...(item.isExternal && { target: '_blank', rel: 'noopener noreferrer' })}>{item.title}</a>
                                </li>
                            ))
                        }
                    </ul>
